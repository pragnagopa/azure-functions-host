...
namespace Microsoft.Azure.WebJobs.Script.Tests.Rpc
{
    public class FunctionDispatcherTests
    {
        ...
        public async void Starting_MultipleJobhostChannels_Succeeds()
        {
            ...
            RpcFunctionInvokeDispatcher functionDispatcher = GetTestFunctionDispatcher(expectedProcessCount.ToString());
        ...
        public async void Starting_MultipleWebhostChannels_Succeeds()
        {
            ...
            RpcFunctionInvokeDispatcher functionDispatcher = GetTestFunctionDispatcher(expectedProcessCount.ToString(), true);
        ...
        {
            RpcFunctionInvokeDispatcher functionDispatcher = GetTestFunctionDispatcher();
        ...
        public async Task ShutdownChannels_NoFunctions()
        {
            ...
            RpcFunctionInvokeDispatcher functionDispatcher = GetTestFunctionDispatcher(mockwebHostLanguageWorkerChannelManager: mockLanguageWorkerChannelManager);
            Assert.Equal(FunctionDispatcherState.Default, functionDispatcher.State);
            await functionDispatcher.InitializeAsync(new List<FunctionMetadata>());
        ...
        {
            RpcFunctionInvokeDispatcher functionDispatcher = GetTestFunctionDispatcher("1");
            await functionDispatcher.InitializeAsync(GetTestFunctionsList(LanguageWorkerConstants.NodeLanguageWorkerName));
        ...
        {
            RpcFunctionInvokeDispatcher functionDispatcher = GetTestFunctionDispatcher(throwOnProcessStartUp: true, addWebhostChannel: true);
            await functionDispatcher.InitializeAsync(GetTestFunctionsList(LanguageWorkerConstants.JavaLanguageWorkerName));
            var testLogs = _testLogger.GetLogMessages();
            Assert.False(testLogs.Any(m => m.FormattedMessage.Contains("Removing errored webhost language worker channel for runtime")));
        }

        private static RpcFunctionInvokeDispatcher GetTestFunctionDispatcher(string maxProcessCountValue = null, bool addWebhostChannel = false, Mock<IWebHostLanguageWorkerChannelManager> mockwebHostLanguageWorkerChannelManager = null, bool throwOnProcessStartUp = false)
        {
            ...
            return new RpcFunctionInvokeDispatcher(scriptOptions,
        ...
        private async Task<int> WaitForJobhostWorkerChannelsToStartup(RpcFunctionInvokeDispatcher functionDispatcher, int expectedCount)
        {
